 ...
 **/
static void
xnl_debug_ignore_cb (const gchar *log_domain, GLogLevelFlags log_level,
...
/**
 * xnl_debug_handler_cb:
 **/
static void
xnl_debug_handler_cb (const gchar *log_domain, GLogLevelFlags log_level,
		     const gchar *message, gpointer user_data)
{
	gchar str_time[255];
	time_t the_time;
...
/**
 * xnl_debug_pre_parse_hook:
 */
static gboolean
xnl_debug_pre_parse_hook (GOptionContext *context, GOptionGroup *group, gpointer data, GError **error)
{
...
/**
 * xnl_debug_add_log_domain:
 */
void
xnl_debug_add_log_domain (const gchar *log_domain)
{
	if (_verbose) {
		...
		g_log_set_handler (log_domain,
				   ...
				   G_LOG_LEVEL_DEBUG |
				   G_LOG_LEVEL_WARNING,
				   xnl_debug_handler_cb, NULL);
	} else {
		/* hide all debugging */
		g_log_set_handler (log_domain,
				   G_LOG_LEVEL_DEBUG | G_LOG_LEVEL_WARNING,
				   xnl_debug_ignore_cb, NULL);
	}
}

/**
 * xnl_debug_post_parse_hook:
 */
static gboolean
xnl_debug_post_parse_hook (GOptionContext *context, GOptionGroup *group, gpointer data, GError **error)
{
	/* verbose? */
	xnl_debug_add_log_domain (G_LOG_DOMAIN);
 ...
 */
GOptionGroup *
xnl_debug_get_option_group (void)
{
	GOptionGroup *group;
	group = g_option_group_new ("debug", _("Debugging Options"), _("Show debugging options"), NULL, NULL);
	g_option_group_set_parse_hooks (group, xnl_debug_pre_parse_hook, xnl_debug_post_parse_hook);
	return group;
}
